...
import { DrawerForm as ProDrawerForm, ProFormText } from '@ant-design/pro-components';
...
export const DrawerFrom = <T extends BasicFormData>({
  ...
  formItems.push(
    <div key="元数据" hidden={section !== '元数据'}>
      <ProFormText
        ...
        required
      />
      <ProFormMap name={['metadata', 'annotations']} label="注解" tooltip="metadata.annotations" />
      <ProFormMap name={['metadata', 'labels']} label="标签" tooltip="metadata.labels" />
    </div>
  );
  ...
  props.onFinish = async (value) => {
    ...
    //   await api?.replace(data.metadata.name as any, data as any);
    // } else {
