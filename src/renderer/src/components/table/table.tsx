import type { DrawerFromProps } from '../form';
import { DrawerFrom } from '../form';
...
export const ProTable = <T extends BasicTableData>({
  ...
  if (formAction.create) {
    const initialValues = lodash.merge(lodash.cloneDeep(formProps?.initialValues || {}), {
      metadata: { namespace: activeNamespace }
    } as T);
...
export const ProMenuTable = <T extends BasicTableData>({
  ...
  // 定义表格参数
  const tableProps: ProTableProps<T> = commonMenuProps ? lodash.cloneDeep(commonMenuProps) : {};
  ...
  // 公共列字段
  const columns = [...(prevColumns || []), ...(tableProps.columns || []), ...(nextColumns || [])];
